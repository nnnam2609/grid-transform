...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Threaded test exporter for warping a full ArtSpeech session into one nnUNet target frame space. "
            "This keeps the current exporter unchanged and adds parallel frame precomputation for testing."
        )
    )
    parser.add_argument(
        "--annotation-speaker",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of threaded workers used for frame precomputation.",
    )
    parser.add_argument(
        "--prefetch",
        type=int,
        default=0,
        help="Number of in-flight frames to queue. Use 0 for the default heuristic (workers * 3).",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    summary = run_session_warp_to_target_threaded(
